...
Is it possible to implement `foo`? Trivially, without throwing an error the answer is *no*, as `never` is a type that contains no values, so it’s impossible for *any* function to produce a value of type `never`.[^1]
